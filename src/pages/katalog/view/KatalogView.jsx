...
function AtkCard({ atk, presenter }) {
  ...
  const gambarUtama = atk?.gambar_utama_url || atk?.gambar_utama;
    ...
    >
      ...
      <Transition.Root show={showSuccessModal} as={Fragment}>
        ...
        >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <Dialog.Panel className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full text-center flex flex-col items-center gap-4">
                <img
                  src={
                    gambarUtama
                      ? gambarUtama
                      : "https://placehold.co/120x90/e2e8f0/94a3b8?text=Gambar+ATK"
                  }
                  alt={namaAtkDisplay}
                  className="w-28 h-20 object-cover rounded-lg mx-auto border border-slate-200 shadow"
                  style={{ marginBottom: 12 }}
                />
                <ShoppingCartIcon className="w-10 h-10 text-rose-500 mb-2 mx-auto" />
                ...
                <button
                  ...
                  className="mt-2 px-5 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg font-semibold shadow"
                >
                  Tutup
                </button>
              </Dialog.Panel>
      ...
      <div className="relative overflow-hidden aspect-[4/3] cursor-pointer">
        ...
        <div className="absolute top-4 left-4 z-10 flex flex-col space-y-2">
          {kategoriNama && (
            <span className="bg-rose-600/95 group-hover:bg-rose-700/95 backdrop-blur-sm text-white text-[10px] font-bold px-3 py-1.5 rounded-full shadow-lg tracking-wide flex items-center transition-colors duration-200">
          ...
          {statusKetersediaan && (
            <span
              className={cn(
                "text-[10px] font-bold px-3 py-1.5 rounded-full shadow-lg tracking-wide backdrop-blur-sm bg-rose-500 group-hover:bg-rose-600 text-white transition-colors duration-200"
              )}
            >
        ...
        {/* Hapus tombol tambah ke keranjang di tengah gambar saat hover */}
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="mt-auto grid grid-cols-2 gap-3">
          <button
            onClick={() => navigate(`/atk/${atk.slug}`)}
            className="view-detail-button w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold py-3 px-4 rounded-2xl shadow-sm transition-all duration-200 flex items-center justify-center text-sm focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-1"
      ...
      <div className="container mx-auto px-4 sm:px-5 lg:px-6 py-4 md:py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
          {loading
            ...
            : atkList.length > 0
            ? atkList.map((atk) => (
                <AtkCard key={atk.id} atk={atk} presenter={presenter} />
              ))
            : !error && (
                <div
